...
#### AT cell analysis: Reintegration; UMAPs, diffusion maps, dot, feature and violin plots; DGE and hypergeometric test; DATP cell state fractions
library(Seurat)
library(destiny)
library(dplyr)
library(ggplot2)
...
DimPlot(patient, reduction = 'umap', label = F, group.by = 'cell_type_ourdatp', shuffle = T, raster = T, cols = c('#0072B2', '#0AB45A', '#FA5078')) + NoLegend()
...
# Feature plots
FeaturePlot(patient, features = c('UP_in_DATPs1', 'DATP_short1', 'DATP_long1', 'our_DATP_sig1'), min.cutoff = 'q05', max.cutoff = 'q95', order = T, raster = T)
...
FeatureScatter(patient, feature1 = 'G2M.Score', feature2 = 'S.Score', group.by = 'cell_type_ourdatp', cols = c('#0072B2', '#0AB45A', '#FA5078'), pt.size = 0.2)
...
VlnPlot(object = subset(patient, cell_type_fine == 'AT1'), features = c('CAV1', 'ETV5', 'SFTPB', 'our_DATP_sig1'), ncol = 4, pt.size = 0, group.by = 'cell_type_fine', cols = colors, split.by = 'group')
VlnPlot(object = subset(patient, cell_type_fine == 'AT2'), features = c('CAV1', 'ETV5', 'SFTPB', 'our_DATP_sig1'), ncol = 4, pt.size = 0, group.by = 'cell_type_fine', cols = colors, split.by = 'group')
VlnPlot(object = patient, features = c('CAV1', 'ETV5', 'SFTPB', 'our_DATP_sig1'), ncol = 4, pt.size = 0, cols = colors, group.by = 'group')
DotPlot(patient, features = c('AT11', 'AT21', 'DATP_short1', 'our_DATP_sig1'), group.by = 'cell_type_ourdatp', dot.scale = 7) + scale_color_viridis() + RotatedAxis() + coord_flip()
...
legend('bottomright', inset = c(0, 0), legend = levels(as.factor(es@phenoData@data$cell_type_fine)), pch = 16, col = as.factor(levels(as.factor(es@phenoData@data$cell_type_fine))), bty = 'n')
...
legend('bottomright', inset = c(0, 0), legend = levels(as.factor(es@phenoData@data$cell_type_ourdatp)), pch = 16, col = as.factor(levels(as.factor(es@phenoData@data$cell_type_ourdatp))), bty = 'n')
...
### DATP signature generation 
# Add scaled data for DEG
DefaultAssay(patient) <- 'RNA'
...
### DATP quantification
df_tobesummed_fine = data.frame(orig.ident = patient$orig.ident, 
                                group = patient$group, 
                                cell_type_ourdatp = patient$cell_type_ourdatp)
...
write.csv(prop.table(table(subset(patient, group == 'COVID-19')$cell_type_ourdatp)), 'data/lungs_all/cell_type_fraction_datp_among_AT1AT2_cov.csv', row.names = F)
write.csv(prop.table(table(subset(patient, group == 'Control')$cell_type_ourdatp)), 'data/lungs_all/cell_type_fraction_datp_among_AT1AT2_ctr.csv', row.names = F)